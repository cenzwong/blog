...
This website is a state-of-the-art static site hosted entirely for free on **GitHub Pages**, featuring a fully serverless, client-side search engine. It runs **SQLite compiled to WebAssembly (Wasm)** in the browser, querying a pre-compiled search index via **HTTP Range Requests** (fetching only the exact database pages needed, rather than downloading the whole database file!).
...
2. **`scripts/ingest.py`**: A helper Python compiler script. It reads your Markdown articles, compiles them into a compact SQLite database, builds rapid search indexes, and saves it in `public/search_index.db`.
3. **`src/`**: The frontend design! `src/index.css` is a stunning premium space-dark stylesheet. `src/main.js` handles routing pages and runs real-time SQLite SQL queries.
...
| `npm run ingest` | Compiles your Markdown files into the SQLite database. | Every time you add, edit, or delete an article. |
...
Enjoy writing and building with Wasm-SQLite! If you ever have any questions about custom features, styles, or queries, just ask! 💻✨
