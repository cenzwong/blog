...
Please report this to https://github.com/markedjs/marked.`,e){let e=`<p>An error occurred:</p><pre>`+l(n.message+``,!0)+`</pre>`;return t?Promise.resolve(e):e}if(t)return Promise.reject(n);throw n}}},K=new G;function q(e,t){return K.parse(e,t)}q.options=q.setOptions=function(e){return K.setOptions(e),q.defaults=K.defaults,n(q.defaults),q},q.getDefaults=e,q.defaults=t,q.use=function(...e){return K.use(...e),q.defaults=K.defaults,n(q.defaults),q},q.walkTokens=function(e,t){return K.walkTokens(e,t)},q.parseInline=K.parseInline,q.Parser=U,q.parser=U.parse,q.Renderer=V,q.TextRenderer=H,q.Lexer=B,q.lexer=B.lex,q.Tokenizer=y,q.Hooks=W,q.parse=q,q.options,q.setOptions,q.use,q.walkTokens,q.parseInline,U.parse,B.lex;var Ee=new G,J=[],Y=null,De=document.getElementById(`sqlite-status`),Oe=document.getElementById(`sqlite-status-text`),X=document.getElementById(`main-view`);function Z(e,t){De.className=`engine-badge status-${e}`,Oe.textContent=t}function ke(e){return e?e.replace(/\$\$[\s\S]*?\$\$/g,``).replace(/\$[^\$]+\$/g,``).replace(/```[\s\S]*?```/g,``).replace(/`([^`]+)`/g,`$1`).replace(/\[([^\]]+)\]\([^\)]+\)/g,`$1`).replace(/[*_#\-+>]/g,` `).replace(/\s+/g,` `).trim():``}async function Ae(){try{Z(`loading`,`Index Booting...`);let e=new URL(`/blog/search_index.json`,window.location.origin).toString();console.log(`Connecting to remote search index: ${e}`);let t=await fetch(e);if(!t.ok)throw Error(`Failed to fetch search index: ${t.status} ${t.statusText}`);J=await t.json(),console.log(`Search index successfully loaded with ${J.length} posts.`),Z(`connected`,`Index Active`),window.addEventListener(`hashchange`,Q),Q()}catch(e){console.error(`Failed to initialize search index:`,e),Z(`error`,`Index Offline`),X.innerHTML=`
    ...
    `}}async function je(){if(Y)return Y;if(!window.loadPyodide)return console.error(`Pyodide script not loaded.`),null;try{return Z(`loading`,`Loading Pyodide...`),Y=await window.loadPyodide({indexURL:`https://cdn.jsdelivr.net/pyodide/v0.25.0/full/`}),Y.runPython(`
      import sys
      import io
      sys.stdout = io.StringIO()
    `),Z(`connected`,`Pyodide Active`),Y}catch(e){return console.error(`Failed to initialize Pyodide:`,e),Z(`error`,`Pyodide Failed`),null}}async function Me(e,t){t.textContent=`Loading Pyodide environment and executing...`,t.style.color=`var(--text-secondary)`;let n=await je();if(!n){t.textContent=`Error: Pyodide could not be loaded.`,t.style.color=`var(--danger, #ef4444)`;return}try{n.runPython(`sys.stdout = io.StringIO()`),await n.runPythonAsync(e);let r=n.runPython(`sys.stdout.getvalue()`);r.trim()===``?(t.textContent=`[Execution completed with no output]`,t.style.color=`var(--text-muted)`):(t.textContent=r,t.style.color=`var(--success, #10b981)`)}catch(e){t.textContent=e.toString(),t.style.color=`var(--danger, #ef4444)`}}function Q(){let e=window.location.hash||`#/`;document.querySelectorAll(`.nav-link`).forEach(e=>e.classList.remove(`active`)),e===`#/`||e===``?(document.getElementById(`nav-home`)?.classList.add(`active`),Ne()):e===`#/search`?(document.getElementById(`nav-search`)?.classList.add(`active`),Pe()):e===`#/about`?(document.getElementById(`nav-about`)?.classList.add(`active`),Le()):e.startsWith(`#/post/`)?Ie(e.replace(`#/post/`,``)):$()}async function Ne(){X.innerHTML=`
    <div class="loading-view">
      <div class="spinner"></div>
      <p>Fetching posts from JSON index...</p>
    </div>
  `;try{if(J.length===0){X.innerHTML=`
    ...
    `}catch(e){console.error(`Error loading home posts:`,e),X.innerHTML=`<div class="error-view"><h2>Error Loading Posts</h2><p>${e.message}</p></div>`}}async function Pe(){X.innerHTML=`
  ...
  `;let e=document.getElementById(`search-box`),t=document.getElementById(`search-results`),n=document.getElementById(`search-stats`),r=null;e.focus(),e.addEventListener(`input`,e=>{clearTimeout(r);let i=e.target.value;r=setTimeout(()=>{Fe(i,t,n)},150)})}async function Fe(e,t,n){let r=e.trim().toLowerCase();if(!r){n.textContent=``,t.innerHTML=`
      <div class="search-empty-state">
        <h3>Ready to Search</h3>
        <p>Start typing above to trigger real-time client-side keyword search.</p>
      </div>
  ...
  `;try{let t=J.find(t=>t.slug===e);if(!t){$();return}let n=t.tags?t.tags.split(`,`).map(e=>e.trim()):[],r=t.format===`html`?t.content:Ee.parse(t.content);r=r.replace(/\$\$(.*?)\$\$/gs,(e,t)=>{if(window.katex)try{return window.katex.renderToString(t.trim(),{displayMode:!0,throwOnError:!1})}catch(e){console.warn(`KaTeX error:`,e)}return`<div class="math-block">${t.trim()}</div>`}).replace(/\$(.*?)\$/g,(e,t)=>{if(window.katex)try{return window.katex.renderToString(t.trim(),{displayMode:!1,throwOnError:!1})}catch(e){console.warn(`KaTeX error:`,e)}return`<span class="math-inline">${t.trim().replace(/\\times/g,`×`).replace(/\\text\{([^}]+)\}/g,`$1`).replace(/\\frac\{([^}]+)\}\{([^}]+)\}/g,`($1/$2)`).replace(/\\log/g,`log`)}</span>`});let i=``;i=t.author===`Cenz Wong & Gemini AI`?`
    ...
    `,window.hljs&&X.querySelectorAll(`pre code`).forEach(e=>{window.hljs.highlightElement(e)}),X.querySelectorAll(`.py-runner-btn`).forEach(e=>{e.addEventListener(`click`,()=>{let t=e.getAttribute(`data-target`),n=e.getAttribute(`data-output`),r=document.getElementById(t),i=document.getElementById(n);r&&i&&Me(r.innerText||r.textContent,i)})}),window.scrollTo({top:0,behavior:`smooth`})}catch(e){console.error(`Error fetching article:`,e),X.innerHTML=`<div class="error-view"><h2>Failed to Render Article</h2><p>${e.message}</p></div>`}}function Le(){X.innerHTML=`
